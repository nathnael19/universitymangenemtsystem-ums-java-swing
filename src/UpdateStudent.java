...
public class UpdateStudent extends JFrame implements ActionListener {
    ...
    UpdateStudent() {
        ...
        String queryId = "SELECT * FROM `students`";
        ...
        idChoice.addItemListener((ItemEvent ie) -> {
            String queryFirstName = "SELECT * FROM `students` WHERE id='" + idChoice.getSelectedItem() + "'";
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == submitButton) {
            String myId = idlbl.toString();
            String address = addressTextField.getText();
            String phoneNumber = phoneTextField.getText();
            String email = emailTextField.getText();

            // Exeption Handling Method
            while (!(address.isEmpty() || phoneNumber.isEmpty() || email.isEmpty())) {

                MySQLDatabase data = new MySQLDatabase();
//                String query = "UPDATE `students` SET `address`= ?,`phoneNumber`= ?,`email`= ? WHERE `id`= '"+myId+"'";
                String query ="UPDATE `students` SET `address`= '"+address+"',`phoneNumber`= '"+phoneNumber+"',`email`= '"+email+"' WHERE `id`='"+myId+"'";
                try {
                    GetMyData date = new GetMyData();
                    date.statement.executeUpdate(query);
